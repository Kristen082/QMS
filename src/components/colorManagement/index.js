  ...
  confirm({
    ...
    onOk() {
      console.log(check_List)
        //删除事件
        axios.post(`/api/ColorCode/ColorCodeDelete`,{
          "pId": '10',
          "oIds": check_List
        }).then(res=>{
...
export default class User extends Component {
      ...
      columns: [
				{
					...
					key: 'x',
          width: 50,
					render: () => <a onClick={()=>{this.showModal(false)}}>Edit</a>,
				},
        {
          title: 'Color No',
          dataIndex: 'colorNo',
          width: 150,
          sorter: (a, b) => a.colorNo - b.colorNo,
          ...self.getColumnSearchProps('colorNo'),
        },
        {
          title: 'Color Name',
          dataIndex: 'colorName',
          width: 150,
          sorter: (a, b) => a.colorName - b.colorName,
          ...self.getColumnSearchProps('colorName'),
        }
      ]
    }
  ...
  handleReset = clearFilters => {
    ...
    //确定
      ...
      let {code,color}=this.state.fields;//value框里的状态
			this.setState({//转圈
			...
			if(flag){
				axios.post("/api/ColorCode/ColorCodeAdd",{
					"colorNo": color.value,
					"colorDesc": code.value,
    ...
    componentDidMount(){
      axios.post('/api/ColorCode/GetColorCodeList',{
        "isPagination": true,
